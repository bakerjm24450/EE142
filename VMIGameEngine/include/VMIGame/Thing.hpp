...
namespace vmi
{
    ...
    inline void Thing::draw(sf::RenderTarget &target, sf::RenderStates states) const
    {
        ...
        states.transform.scale(sf::Vector2f((float)scale, (float)scale),
                               sf::Vector2f((float)cntr.getX(), (float)cntr.getY()));
        states.transform.rotate(sf::degrees((float)angle),
                                sf::Vector2f((float)cntr.getX(), (float)cntr.getY()));
        states.transform.translate(sf::Vector2f((float)x.getX(), (float)x.getY()));
    ...
    inline const BoundingBox Thing::getBounds() const
    {
        ...
        sf::FloatRect sf_bb(sf::Vector2f((float)bb.getUl().getX(), (float)bb.getUl().getY()),
                            sf::Vector2f((float)bb.getWidth(), (float)bb.getHeight()));
        ...
        // build a transform
        sf::Transform t;
        t.scale(sf::Vector2f((float)scale, (float)scale),
                sf::Vector2f((float)cntr.getX(), (float)cntr.getY()));
        t.rotate(sf::degrees((float)angle),
                 sf::Vector2f((float)cntr.getX(), (float)cntr.getY()));
        t.translate(sf::Vector2f((float)x.getX(), (float)x.getY()));

        // transform the bounding box
        sf_bb = t.transformRect(sf_bb);

        // convert it back to our bounding box (ul and lr corners)
        return BoundingBox(Vector2d(sf_bb.position.x, sf_bb.position.y),
                           Vector2d(sf_bb.position.x + sf_bb.size.x,
                                    sf_bb.position.y + sf_bb.size.y));
    }
